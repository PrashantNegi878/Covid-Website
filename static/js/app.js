...
let all_time_chart, recover_rate_chart
...
showConfirmedNew = (total) => {
    document.querySelector('#confirmed-new').textContent = numberWithCommas(total)
}
...
showRecoveredNew = (total) => {
    document.querySelector('#recovered-new').textContent = numberWithCommas(total)
}
...
showDeathsNew = (total) => {
    document.querySelector('#death-new').textContent = numberWithCommas(total)
}
...
loadSummary = async (country) => {
    ...
    showConfirmedNew(summary.NewConfirmed)
    showRecoveredTotal(summary.TotalRecovered)
    showRecoveredNew(summary.NewRecovered)
    showDeathsTotal(summary.TotalDeaths)
    showDeathsNew(summary.NewDeaths)
